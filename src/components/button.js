...
const Button = React.forwardRef(({children, className, link, color = 'bg-gray-900', colorDark = 'bg-white', text = 'text-white', textDark = 'text-black', ...rest}, ref) => {
    const classNames = `cursor-pointer shadow rounded-lg whitespace-nowrap ${color} dark:${colorDark} ${text} dark:${textDark} flex items-center justify-center px-6 py-4 transition transform active:scale-95 focus:outline-none disabled:opacity-50 ${className || ''}`;
